...
export class Question_Pg extends Component {
    constructor(props) {
      super(props);
    ...
    handleTitleClick(e) {
      // axios.get('http://localhost:8000/posts/question/'+e.target.id).then(
                ...
                return (
                  <tr key={index}>
                    ...
                    <td>
                      ...
                      <div>
                        {item.tags.map((i) => { 
                          //tags data should be already populate in question  
                          //so calling .name should get the tag name
                          return (
